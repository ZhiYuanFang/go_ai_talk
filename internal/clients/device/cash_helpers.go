// Package device 出站调用 device-service 的中立 HTTP 客户端。
package device
...
func FetchNonLeafEventCount(ctx context.Context) (int, error) {
	...
	if err != nil {
		glog.Warningf(ctx, "[clients/device] device root-count(non-leaf-count) 调用失败 err=%v", err)
		return 0, gerror.WrapCode(gcode.CodeInternalError, err, "device 一级根计数失败")
	}
	defer resp.Body.Close()
	body, _ := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
	if resp.StatusCode != http.StatusOK {
		glog.Warningf(ctx, "[clients/device] device root-count HTTP=%d body=%s", resp.StatusCode, string(body))
...
func FetchDeviceNoByWxID(ctx context.Context, wxID int64) (string, error) {
	...
	resp, err := client.Do(req)
	if err != nil {
		glog.Warningf(ctx, "[clients/device] device device-no-by-wx-id 调用失败 wxId=%d err=%v", wxID, err)
		return "", gerror.WrapCode(gcode.CodeInternalError, err, "查询码主人设备失败")
	}
	defer resp.Body.Close()
	body, _ := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
	if resp.StatusCode != http.StatusOK {
		glog.Warningf(ctx, "[clients/device] device device-no-by-wx-id HTTP=%d body=%s", resp.StatusCode, string(body))
