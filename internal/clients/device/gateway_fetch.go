...
func FetchDeviceNoByWxIDViaGClient(ctx context.Context, wxID int64) (string, error) {
	...
	resp, err := gclient.New().SetHeader(httpmeta.HeaderDeviceGatewayInternalSecret, secret).Get(ctx, url)
	if err != nil {
		return "", err
	}
