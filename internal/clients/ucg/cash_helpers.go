// Package ucg 出站调用 ucg-service 的中立 HTTP 客户端。
package ucg
...
func NotifyUcgInviteAcquisition(ctx context.Context, ownerWxID int64, ref string) error {
	...
	if err != nil {
		glog.Warningf(ctx, "[clients/ucg] ucg force/acquire 调用失败 owner=%d err=%v", ownerWxID, err)
...
func FetchUcgNicknames(ctx context.Context, wxIDs []int64) (map[int64]string, error) {
	...
	resp, err := client.Do(req)
	if err != nil {
		glog.Warningf(ctx, "[clients/ucg] ucg profiles/batch 失败 err=%v", err)
		return out, err
	}
	defer resp.Body.Close()
	body, _ := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
