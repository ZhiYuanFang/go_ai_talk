...
func delegateGetProfile(ctx context.Context, deviceNo string) (birthday string, sex int, err error) {
	t := contracts.ResolveHTTPTargets()
	var resp struct {
	...
	return strings.TrimSpace(resp.Birthday), resp.Sex, nil
}

func delegateSaveProfile(ctx context.Context, deviceNo, birthday string, sex int) error {
	t := contracts.ResolveHTTPTargets()
