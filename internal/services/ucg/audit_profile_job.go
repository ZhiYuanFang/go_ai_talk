...
// auditProfileJobFromEvent MQ 消费者：Green + CAS job + apply profile。
func auditProfileJobFromEvent(ctx context.Context, jobID uint64, auditVersion int) error {
	row, err := dao.UcgProfileAuditJob.Ctx(ctx).Where(dao.UcgProfileAuditJob.Columns().Id, jobID).One()
	if err != nil {
		return err
	}
	if row.IsEmpty() {
	...
	if job.Nickname != "" {
		if verdict, mErr := moderator.ModerateText(ctx, "nickname_detection", job.Nickname); mErr != nil {
			return mErr
		} else if !verdict.Pass {
			return rejectProfileJobCAS(ctx, jobID, auditVersion, verdict.Reason)
		}
	}
	if job.Bio != "" {
		if verdict, mErr := moderator.ModerateText(ctx, "comment_detection", job.Bio); mErr != nil {
			return mErr
		} else if !verdict.Pass {
			return rejectProfileJobCAS(ctx, jobID, auditVersion, verdict.Reason)
		}
	...
	return approveProfileJobCAS(ctx, job, auditVersion)
}
