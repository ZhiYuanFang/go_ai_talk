...
func (s *VoiceService) chatWithResult(ctx context.Context, deviceNo, transcript string) (string, string, bool, bool, error) {
	...
	action := entity.Action{
		...
		TargetType: strings.TrimSpace(intent.TargetType),
	}
	if action.Name == "" {
...
func (s *VoiceService) handleUnifiedIntentAction(ctx context.Context, deviceNo, normalizedTranscript string, action entity.Action, events []entity.Event, intent deepSeekUnifiedIntent) (finalReply string, exit bool, finishTalk bool, err error) {
	...
	case ActionTargetTypeEnd:
		...
		if lastEvent.EndTime == "" && lastEvent.EventId > 0 {
			_, _ = dao.History.Ctx(ctx).Where("device_no", deviceNo).Where("event_id", lastEvent.EventId).Update(g.Map{"end_time": nowTime})
	...
	case "end": //结束记录计时动作，自动补结束时间
		...
		if lastEvent.EventId == event.Id {
			...
			if lastEvent.EndTime == "" {
				_, updateErr := dao.History.Ctx(ctx).Where("device_no", deviceNo).Where("event_id", lastEvent.EventId).Update(g.Map{"end_time": nowTime})
				if updateErr != nil {
