...
7. **商业功能开通（commercial-feature-entitlement + prediction-default-invite-admin-ux）**：`EnsureSchema` 建 `feature_*` 表并种子 `prediction_unlock`（含 `default_allowed_count`；设备表含 `full_access` / `full_access_expires_at`）。cash 须配置 `HISTORY_SERVICE_URL`（调 history `GET /history/internal/api/feeding-day-stats`，共享 `DEVICE_GATEWAY_INTERNAL_SECRET`）；**禁止** cash 直连 history 库。App：`GET /cash/app/api/ucg/eligibility`、`GET /cash/app/api/feature/catalog`（须登录+绑机；查询不计入 usage）；预测项 `allowedCount`：**有效条数 = 默认 + 永久累加**；邀请码临时全开哨兵 **`-1`**（客户端须识别）。开通意图 `POST .../feature/orders`、`.../invite-codes/redeem`、`.../ad/complete` **计入** usage。支付回调与 VIP **共用** `/cash/app/api/vip/alipay/notify`（及 Apple verify），按 `feature_order`/`vip_order` 分流。Hub：「开通功能管理」`/device/admin/cash-feature-admin.html`、「邀请码管理」`/device/admin/cash-invite-code-admin.html`（静态资源变更后须强制刷新）。
