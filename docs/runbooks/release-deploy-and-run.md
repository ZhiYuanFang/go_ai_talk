...
7. **商业功能开通 + 喂养资格场景**：`EnsureSchema` 建 `feature_*` 与 `feeding_eligibility_scene`（种子 `ucg_entry` 7/10、`care_alert_entry` 2/10）。UCG / 值得留意连续有效日判定 **均在 cash-service**；history 仅按日计数；窗口=`requiredDays`。App：`GET /cash/app/api/ucg/eligibility`、`GET /cash/app/api/care-alert/eligibility`（查询不计入 usage）。Hub：「喂养资格门槛」`/device/admin/cash-feeding-eligibility-admin.html`（Ctrl+F5）。
