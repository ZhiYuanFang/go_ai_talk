# OpenSpec 基线规格 v2.0.3

> 本文件由 `openspec/specs` 下全部 capability 规格于 **v2.0.3** 合并而成，作为该版本的确定性规则基线，便于按版本查阅。
...
系统 SHALL 将 RabbitMQ 作为唯一跨服务事件通道，并且 SHALL NOT 保留必需事件流程中的 MQ 关闭分支。运行时必需事件流程中的 MQ 发布失败语义保持不变，但 TextChat 的 `voice.task.requested` 发布除外（见 `voice-textchat-resilience`）。启动期探活策略 SHALL 按进程角色区分：API 类进程 MAY 在 MQ 不可达时仍启动；worker-service SHALL 在 MQ 不可达时启动失败。
