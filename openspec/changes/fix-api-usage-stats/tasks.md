...
## 8. 补刀：Redis HGETALL 读路径（flat []string）

- [x] 8.1 `usagestats/store.go`：`redisHashToMap` 对 `*gvar.Var` 优先 `MapStrStr()`，保留 map / `[]interface{}` fallback
- [x] 8.2 确认 `ListAPIs` / `ListUsersForAPI` / `ListAPIsForUser` 三条读路径均经 `redisHashToMap`（无旁路）
- [x] 8.3 test 验证：Redis `HGETALL gw:usage:d:{today}:g` 有 field 时，`GET /device/admin/api/usage/list?days=7` 返回非空 `list`；「按用户」wxId 15 有明细（部署 gateway-app 后验收；test Redis 已有 `gw:usage:d:20260611:g` / `w:15`）

## 9. 校验（补刀）

- [x] 9.1 `openspec validate fix-api-usage-stats --strict`
- [x] 9.2 `go build ./...`
