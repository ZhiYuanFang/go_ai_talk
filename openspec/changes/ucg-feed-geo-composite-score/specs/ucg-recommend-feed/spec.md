...
Hot zone MUST 定义为 `published_at >= round_hot_cutoff`；冷区帖 MUST 依赖 MQ 更新 ZSET score。Feed 读路径 MUST 合并：(a) GEO 半径候选（有坐标帖）；(b) ZSET 中不在 GEO 索引的无坐标帖。排序 MUST 为 `finalScore DESC, postId DESC`。
...
GEO 候选 MUST 使用默认半径 50km，不足 `pageSize` 时按阶梯扩展：`50 → 100 → 200 → 500 → unlimited` km。cursor MUST 冻结首屏 `lat/lng/radiusKm/geoOffset/sessionId` 以保证 session 内一致。
...
- **WHEN** published 帖无 lat/lng 不在 GEO 索引
- **THEN** 该帖 MUST 仍从 ZSET 参与候选且按 baseScore 排序，响应 MUST NOT 含 `distanceMeters`
...
- **THEN** 系统 MUST 扩大至下一半径阶梯继续拉取直至满足或达 unlimited
