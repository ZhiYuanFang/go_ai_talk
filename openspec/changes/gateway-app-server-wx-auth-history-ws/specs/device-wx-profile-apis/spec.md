...
- **WHEN** 首次出现的 wxCode 调用登录接口
- **THEN** 系统 SHALL 创建 wx 行并返回 is_new_user 为真（或等价字段），且 device_no 字段 MAY 为空

#### Scenario: 已存在用户

- **WHEN** wxCode 已存在
- **THEN** 系统 SHALL 返回已有 wxId 与 wxCode，并在已绑定设备时返回 device_no
...
- **WHEN** 请求头包含有效 wxCode 且 body 中 deviceNo 合法
...
- **WHEN** wxCode 有效且当前 wx 行未绑定 device_no，且 birthday/sex 合法
...
- **WHEN** wxCode 有效且 wx 已绑定 device_no
...
- **THEN** 响应 SHALL 包含与该 id 对应的 wxCode
...
device-service 对上述只读或高频读路径（含 id→wxCode）SHALL 可选使用 Redis 缓存；在更新 wx 绑定或影响 wxCode 映射的写操作成功后，SHALL 使相关缓存键失效。

#### Scenario: 绑定后缓存一致性

- **WHEN** bindwx 成功完成
- **THEN** 与该 wx 行相关的 id→wxCode 或 wxCode 详情缓存 SHALL 被删除或失效，使得后续读取获得新状态
