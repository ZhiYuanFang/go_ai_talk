...
- `gateway-app-server`：App 网关进程、鉴权中间件、`X-Internal-Wx-Code` 注入、登录与刷新、版本检查 API、历史 WS Hub、Redis Pub/Sub 订阅与相关缓存键语义。
- `device-wx-profile-apis`：device 上 wx 绑定、画像自动保存、wx 详情、微信登录（仅业务）、按主键 id 解析 wxCode 等契约与安全边界（内部头仅可信网络）。
